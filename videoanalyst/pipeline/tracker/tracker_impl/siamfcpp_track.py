...
class SiamFCppTracker(PipelineBase):
    ...
    def addState(self, name, values):
        try:
            self._state[name].append(values)
        except KeyError:
            self._state[name]=[]
            self._state[name].append(values)
    
    def addNew(self, im, state):
        rect = state[0:4]  # bbox in xywh format is given for initialization in case of tracking
        box = xywh2cxywh(rect)
        ...
        self.classes.append(state[-1])
        self.addState('z_crop', im_z_crop)
        self.addState('avg_chans', avg_chans)
        self.addState('features', features)
        self.addState('window', window)
        self.addState('state', (target_pos, target_sz))
        self.addState('is_lost',False)

    def track(self,index,im_x,target_pos,target_sz,features,update_state=False,**kwargs):
        if 'avg_chans' in kwargs:
            avg_chans = kwargs['avg_chans']
        else:
            avg_chans = self._state['avg_chans'][index]
        ...
        self._state['is_lost'][index]= is_lost
        best_pscore_id, pscore, penalty = self._postprocess_score(
            index, score, box_wh, target_sz, scale_x)
        # box post-processing
        ...
        self.addState('x_crop', im_x_crop)
        bbox_pred_in_crop = np.rint(box[best_pscore_id]).astype(np.int)
        self.addState('bbox_pred_in_crop', bbox_pred_in_crop)
        # record optional mid-level info
        if update_state:
            self.addState('score', score)
            self.addState('pscore', pscore)
            self.addState('all_box', box)
            self.addState('cls',cls)
            self.addState('ctr', ctr)
    ...
    def update(self, im):
        ...
        result = []
        for i in range(len(self._state['state'])):
            target_pos_prior, target_sz_prior = self._state['state'][i]
            features = self._state['features'][i]
            if self._state['is_lost'][i]:
                continue
            # forward inference to estimate new state
            target_pos, target_sz = self.track(i,im,target_pos_prior,target_sz_prior,features,update_state=True)
            if self._state['is_lost'][i]:
                continue
            # save underlying state
            # self.state['target_pos'], self.state['target_sz'] = target_pos, target_sz
            self._state['state'][i] = target_pos, target_sz

            # return rect format
            track_rect = cxywh2xywh(np.concatenate([target_pos, target_sz],axis=-1))
            result.append((i, track_rect, self._state['is_lost'][i], self.classes[i]))
        return result

    # ======== tracking processes ======== #

    def _postprocess_score(self, index, score, box_wh, target_sz, scale_x):
        r"""
        Perform SiameseRPN-based tracker's post-processing of score
        :param score: (HW, ), score predictionavg_chans
        ...
        pscore = pscore * (
            1 - window_influence) + self._state['window'][index] * window_influence
